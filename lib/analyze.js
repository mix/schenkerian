...
const CHEERIO_FIND_IMG_ELEMENT_PATHS = [
  'meta[property="og:image"]',
  'meta[property="twitter:image:src"]',
  'meta[name="twitter:image"]',
  'meta[name="twitter:image:src"]'
].join(', ')
const CHEERIO_FIND_TITLE_ELEMENT_PATHS = [
...
function gatherMetaData(url, body) {
  ...
  .then(parsedBody => {
    ...
    const extractCanonical = function() {
      const canonical = getSimpleValue('link[rel="canonical"]', 'href')
      if (canonical && canonical.startsWith('/')) {
        const parsedUrl = Url.parse(url)
        return `${parsedUrl.protocol}//${parsedUrl.host}${canonical}`
      }
      return canonical
    }
    return {
